...
final class HouseTests: XCTestCase {
    ...
    func testDecodingOfSingleHouse() throws {
        guard let path = Bundle(for: type(of: self)).path(forResource: "house7", ofType: "json") else {
    ...
    func testDecodingArrayOfHouses() throws {
        guard let path = Bundle(for: type(of: self)).path(forResource: "houses", ofType: "json") else {
